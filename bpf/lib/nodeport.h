						     ...
						     bool *from_endpoint)
{
	...
	if (dir == NAT_DIR_EGRESS) {
		...
		// TODO(brb) use skb mark to determine if src is a local
		// endpoint
...
#define NODEPORT_DO_NAT_IPV4(ADDR, NDIR)							\
	({											\
		...
		int ____ret = nodeport_nat_ipv4_needed(skb, (ADDR), (NDIR), &from_endpoint) ?	\
			      snat_v4_process(skb, (NDIR), &target, from_endpoint) : TC_ACT_OK; \
		if (____ret == NAT_PUNT_TO_STACK)						\
			____ret = TC_ACT_OK;							\
		____ret;									\
	})
...
#endif
	ret = snat_v4_process(skb, dir, &target, false);
	if (IS_ERR(ret)) {
		/* In case of no mapping, recircle back to main path. SNAT is very
		 * expensive in terms of instructions (since we don't have BPF to
		 * BPF calls as we use tail calls) and complexity, hence this is
