...
const (
	neighbors4MapName = "cilium_nodeport_neigh4"
	neighbors6MapName = "cilium_nodeport_neigh6"
)
...
func InitMaps(ipv4, ipv6 bool) error {
	if ipv4 {
		neighbors4Map := bpf.NewMap(neighbors4MapName,
			bpf.MapTypeLRUHash,
			&Key4{},
		...
		if _, err := neighbors4Map.OpenOrCreate(); err != nil {
			return err
		}
		if err := neighbors4Map.Close(); err != nil {
			return err
		}
	}
	if ipv6 {
		neighbors6Map := bpf.NewMap(neighbors6MapName,
			bpf.MapTypeLRUHash,
			&Key6{},
			int(unsafe.Sizeof(Key4{})),
			&Value{},
		...
		if _, err := neighbors6Map.OpenOrCreate(); err != nil {
			return err
		}
		if err := neighbors6Map.Close(); err != nil {
			return err
		}
	}
	return nil
